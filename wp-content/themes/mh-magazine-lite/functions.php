...
function are_we_home_yet(){
	if ( is_home() ) {
		...
		$is_download = $_SERVER['QUERY_STRING'];

		if ($is_download == 'download'){
			if( $detect->isiOS() ){
	 			Redirect('https://itunes.apple.com/us/app/khmer-news-live-24/id1440587029', false);
			}

			if( $detect->isAndroidOS() ){
		 		Redirect('https://play.google.com/store/apps/details?id=com.khmernewslive24.app', false);
			}	
		}
...
// function filter_single_template( $pp_event_single_template, $int ) { 

//     return $pp_event_single_template; 
