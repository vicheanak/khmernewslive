...
function ag_filter_post_json($response, $post, $request) {
    $categories = wp_get_object_terms($post->ID, 'category');
    $response->data['category_name'] = [];

    // $original_content = get_post_meta($post->ID, 'original_content');
    // $response->data['original_content'] = '';
    // $response->data['the_content'] = '';
    // $response->data['app_link'] = '';

    // $app_link = get_post_meta($post->ID, 'app_link');

    // if ($original_content){
    // 	$response->data['original_content'] = preg_replace( '/(width|height)="\d*"\s/', "", $original_content);
    // }

    // if (count($app_link)){
    // 	$response->data['app_link'] = $app_link[0];
    // }
