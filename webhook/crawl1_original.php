...
require( '/var/www/khmernewslive_original/wp-load.php' );
require_once('/var/www/khmernewslive_original/wp-admin/includes/media.php');
require_once('/var/www/khmernewslive_original/wp-admin/includes/file.php');
require_once('/var/www/khmernewslive_original/wp-admin/includes/image.php');
...
$links_news = array('http://www.topnews4khmer.com/author/sothea',
'http://www.topnews4khmer.com/author/soro',
'http://www.topnews4khmer.com/author/chanthorn',
'http://www.topnews4khmer.com/author/sreydy');



// TopNews4Khmer.com 
$crawler_topnews4khmer = $client->request('GET', $links_news[0]);

$links = $crawler_topnews4khmer->filter('h3.entry-title > a')->each(function($node_link){
	...
	$args = array("meta_key" => "source_link", "meta_value" =>$post_link);
	...
	if (count($posts) < 1){
		...
		$crawler->filter('div.entry-content.mh-clearfix > p')->each(function ($node) {
		...
		try{
			$image = $crawler->filter('div.entry-content.mh-clearfix > p > img')->each(function($node){
				$image = $node->attr('src');
				$GLOBALS['post_content'][] = Generate_Image($image);	
			});
		...
		try{
			$fb_video = $crawler->filter('div.entry-content.mh-clearfix > p > iframe')->first();
		...
		try{
			$image = $crawler->filter('div.entry-content.mh-clearfix > figure.entry-thumbnail > img')->eq(0);
...
// TopNews4Khmer.com 
$crawler_topnews4khmer = $client->request('GET', $links_news[1]);
...
// TopNews4Khmer.com 
$crawler_topnews4khmer = $client->request('GET', $links_news[2]);
...
// TopNews4Khmer.com 
$crawler_topnews4khmer = $client->request('GET', $links_news[3]);
